...
class Quiz(commands.Cog):
    ...
    async def _iniciar_quiz(self, guild, channel, autor, perguntas:int=10):
        ...
        self.sessions[key]={'active':True,'scores':{},'total':len(qs)}
        for idx,q in enumerate(qs,1):
            if not self.sessions.get(key,{}).get('active'): break
            opts=list(enumerate(q['o'])); random.shuffle(opts); correta=[i for i,(orig,_) in enumerate(opts) if orig==q['a']][0]
            letras='ABCD'; desc='\n'.join(f'**{letras[i]})** {txt}' for i,(_,txt) in enumerate(opts))
            e=discord.Embed(title=f'📚 Pergunta {idx}/{len(qs)}', description=f"{q['p']}\n\n{desc}\n\n⏳ 45 segundos para responder com A, B, C ou D.", color=CORES['azul'])
            await channel.send(embed=e)
            acertou=set()
            def check(m): return m.channel.id==channel.id and m.content.strip().upper() in list(letras[:len(opts)]) and not m.author.bot
            fim=asyncio.get_event_loop().time()+45
            while asyncio.get_event_loop().time()<fim:
                try: m=await self.bot.wait_for('message', timeout=max(0.1,fim-asyncio.get_event_loop().time()), check=check)
                except asyncio.TimeoutError: break
                escolha=letras.index(m.content.strip().upper())
                if escolha==correta and m.author.id not in acertou:
                    acertou.add(m.author.id); self.sessions[key]['scores'][m.author.id]=self.sessions[key]['scores'].get(m.author.id,0)+1; add_xp(m.author.id, m.author.display_name, 20)
                    try: await m.add_reaction('✅')
                    except Exception: pass
                else:
                    try: await m.add_reaction('❌')
                    except Exception: pass
            if not acertou: await channel.send(f'⏰ Tempo esgotado! Resposta: **{letras[correta]}**')
            await channel.send(self._ranking_text(key, parcial=True))
            await asyncio.sleep(2)
        scores=self.sessions.get(key,{}).get('scores',{})
        total=max(scores.values()) if scores else 0; novo=salvar_quiz_record(guild.id, channel.id, total)
        if scores:
            vencedor=max(scores, key=scores.get); user=guild.get_member(vencedor); add_xp(vencedor, user.display_name if user else str(vencedor), 100)
    ...
    def _ranking_text(self,key,parcial=True):
        scores=self.sessions.get(key,{}).get('scores',{})
        if not scores: return '🏆 Ranking parcial: ninguém pontuou ainda.'
        linhas=[]; guild=self.bot.get_guild(key[0])
        for i,(uid,pts) in enumerate(sorted(scores.items(), key=lambda x:x[1], reverse=True)[:10],1):
            m=guild.get_member(uid) if guild else None; linhas.append(f'**{i}.** {m.display_name if m else uid} — **{pts}** ponto(s)')
    ...
    async def finalizar(self, interaction):
        ...
        if key not in self.sessions: return await interaction.response.send_message('Não tem quiz ativo aqui.', ephemeral=True)
        self.sessions[key]['active']=False; await interaction.response.send_message('🛑 Quiz finalizado. Vou parar após a pergunta atual.')
    ...
    async def quiz_finalizar_prefix(self, ctx):
        ...
        if key not in self.sessions: return await ctx.send('Não tem quiz ativo aqui.')
        self.sessions[key]['active']=False; await ctx.send('🛑 Quiz finalizado. Vou parar após a pergunta atual.')
