...
ANIMAIS_DISPONIVEIS = ['gato', 'cachorro', 'raposa', 'pato', 'panda', 'koala', 'passaro']
ANIMAL_EMOJIS = {
    'gato': '🐱', 'cachorro': '🐶', 'raposa': '🦊',
    'pato': '🦆', 'panda': '🐼', 'koala': '🐨', 'passaro': '🐦',
}
ANIMAL_FALLBACKS = {
    ...
    'koala':   'https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Koala_climbing_tree.jpg/1200px-Koala_climbing_tree.jpg',
    'passaro': 'https://upload.wikimedia.org/wikipedia/commons/thumb/3/32/House_sparrow04.jpg/1200px-House_sparrow04.jpg',
}
...
class HelpView(discord.ui.View):
    CATS = {
        ...
        '🌙 Extras': [
            ...
            '`!animal` / `/animal` — animal fofo aleatório (gato, cachorro, raposa, pato, panda, koala, pássaro)',
...
class Literario(commands.Cog):
            ...
            async with aiohttp.ClientSession(headers={'User-Agent': 'IvyBot/2.0'}) as s:
                ...
                if tipo == 'panda':
                    async with s.get('https://some-random-api.com/animal/panda', timeout=aiohttp.ClientTimeout(total=8)) as r:
                        data = await r.json(); return data['image']
                if tipo == 'koala':
                    async with s.get('https://some-random-api.com/animal/koala', timeout=aiohttp.ClientTimeout(total=8)) as r:
                        data = await r.json(); return data['image']
                if tipo == 'passaro':
                    async with s.get('https://some-random-api.com/animal/bird', timeout=aiohttp.ClientTimeout(total=8)) as r:
                        data = await r.json(); return data['image']
        except Exception:
            pass
    ...
    @app_commands.command(name='animal', description='🐾 Foto de animal fofo aleatório')
    ...
    async def koala_prefix(self, ctx): await self._animal_send(ctx, 'koala')
    @commands.command(name='passaro', aliases=['pássaro', 'bird'])
    async def passaro_prefix(self, ctx): await self._animal_send(ctx, 'passaro')
